...
function StudentQuizzes() {
  ...
  const handleSubmitQuiz = async () => {
    setSubmitting(true);
    try {
      const res = await fetch(`http://127.0.0.1:8000/api/courses/quizzes/${currentQuiz.id}/submit/`, {
        method: "POST",
        headers: getHeaders(),
    ...
    return (
      <div className="max-w-2xl mx-auto text-center space-y-6">
        <div className="bg-white p-12 rounded-3xl border border-slate-200 shadow-sm space-y-6">
          <div className={`w-24 h-24 mx-auto text-white rounded-full flex items-center justify-center mb-4 ${quizResult?.passed ? 'bg-emerald-500' : 'bg-rose-500'}`}>
            <CheckCircle size={48} />
          </div>
          <div>
              ...
              <div key={quiz.id} className="p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 hover:bg-slate-50 transition-colors">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    {quiz.status === 'completed' && quiz.passed && <span className="bg-emerald-100 text-emerald-700 text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">Passed</span>}
                ...
                <div className="w-full md:w-auto">
                  {quiz.status === 'pending' || !quiz.passed ? (
                    <button 
                      ...
                      className="w-full md:w-auto px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-sm shadow-md shadow-blue-600/20 transition-colors flex items-center justify-center gap-2"
                    >
                    ...
                    <div className="flex items-center gap-4">
                      <div className="text-right">
                        ...
                        <p className="text-xl font-black text-emerald-500">{quiz.scoreRaw}/{quiz.questions}</p>
                      </div>
                    </div>
                  )}
                </div>
