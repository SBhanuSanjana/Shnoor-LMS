...
function InstructorQuizzes(){
  ...
  const handleCreateQuiz=async(e)=>{
    ...
    try{
      const res=await fetch(`http://127.0.0.1:8000/api/courses/modules/${selectedModuleId}/quizzes/`,{
        method:"POST",
        headers:getHeaders(),
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
        <div className="p-4 border-b border-slate-200 flex flex-col sm:flex-row justify-between items-center gap-4 bg-slate-50">
          <div className="flex bg-white rounded-lg p-1 border border-slate-200 w-full sm:w-auto">
            <span className="px-4 py-1.5 text-sm font-semibold text-indigo-600">Quiz Listings</span>
          </div>
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            <div className="relative flex-1 sm:w-64">
              ...
              <input 
                ...
                className="w-full pl-9 pr-4 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-300 transition-all bg-white"
              />
            </div>
          </div>
        </div>
                ...
                <div className="flex flex-col md:flex-row justify-between gap-6">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold uppercase tracking-wider bg-emerald-100 text-emerald-700">
                        active
                      </span>
                  ...
                  <div className="flex items-center gap-2">
                    <button 
                      ...
                      className="bg-indigo-50 text-indigo-600 hover:bg-indigo-100 px-4 py-2 rounded-xl text-sm font-semibold transition-colors flex items-center gap-1.5"
                    >
                      <Edit size={16} />
                      Edit in Builder
                    </button>
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                <ClipboardList className="text-indigo-600" /> Create New Quiz
              </h3>
            ...
            <form onSubmit={handleCreateQuiz} className="p-6 space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input 
                    ...
                    className="w-full p-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-100 focus:border-indigo-400 outline-none bg-white" 
              ...
              <div className="pt-4 border-t border-slate-100 flex justify-end gap-3 mt-6">
                ...
                <button type="submit" className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold transition-colors shadow-md">Save and Add Questions</button>
              </div>
            </form>
          </div>
        </div>
