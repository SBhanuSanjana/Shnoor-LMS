...
function CourseBuilder(){
  ...
  const handleAddModule=async(e)=>{
    ...
    try{
      const res=await fetch(`http://127.0.0.1:8000/api/courses/${courseId}/modules/`,{
        method:"POST",
        headers:{...getHeaders(),"Content-Type":"application/json"},
        body:JSON.stringify({title:modTitle,order:modules.length})
      });
      if(res.ok){
        setModTitle('');
  ...
  const handleAddLesson=async(e)=>{
    ...
    try{
      const res=await fetch(`http://127.0.0.1:8000/api/courses/modules/${selectedModId}/lessons/`,{
        method:"POST",
        headers:getHeaders(),
        body:formData
      });
  ...
  const handleAddQuiz=async(e)=>{
    ...
    try{
      const res=await fetch(`http://127.0.0.1:8000/api/courses/modules/${selectedModId}/quizzes/`,{
        method:"POST",
        headers:{...getHeaders(),"Content-Type":"application/json"},
        body:JSON.stringify({title:quizTitle,passing_score:passingScore})
      });
      if(res.ok){
        setQuizTitle('');
        setPassingScore(60);
  ...
  const handleUpdateQuizScore=async(quizId,score)=>{
    try{
      const res=await fetch(`http://127.0.0.1:8000/api/courses/quizzes/${quizId}/`,{
        method:"PUT",
        headers:{...getHeaders(),"Content-Type":"application/json"},
  ...
  const handleAddQuestion=async(e)=>{
    ...
    try{
      const res=await fetch(`http://127.0.0.1:8000/api/courses/quizzes/${selectedQuizId}/questions/`,{
        method:"POST",
        headers:{...getHeaders(),"Content-Type":"application/json"},
        body:JSON.stringify({
          text:qText,
  ...
  const handleCreateFinalQuiz=async()=>{
    try{
      let modId;
      const resMod=await fetch(`http://127.0.0.1:8000/api/courses/${courseId}/modules/`,{
        method:"POST",
        headers:{...getHeaders(),"Content-Type":"application/json"},
        body:JSON.stringify({title:"Final Quiz",order:9999})
      });
      if(resMod.ok){
      ...
      const resQuiz=await fetch(`http://127.0.0.1:8000/api/courses/modules/${modId}/quizzes/`,{
        method:"POST",
        headers:{...getHeaders(),"Content-Type":"application/json"},
        body:JSON.stringify({title:"Final Quiz",passing_score:60})
      });
      if(resQuiz.ok){
  ...
  const handleMoveModule=async(index,direction)=>{
    ...
    try{
      await fetch(`http://127.0.0.1:8000/api/courses/${courseId}/modules/`,{
        method:"PUT",
        headers:{...getHeaders(),"Content-Type":"application/json"},
        body:JSON.stringify({modules:updated.map(m=>({id:m.id,order:m.order}))})
      });
      await loadCourse();
    }catch(e){}
  };
