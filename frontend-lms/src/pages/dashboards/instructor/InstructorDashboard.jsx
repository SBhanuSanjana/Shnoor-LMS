          ...
          <div className="px-6 py-6 border-b border-slate-800">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-blue-400 font-semibold uppercase tracking-wider">Instructor</p>
              </div>
            </div>
          </div>
          <nav className="px-3 py-6 space-y-1.5">
            {menuItems.map((item,index)=>{
              const isActive=location.pathname===item.path||(location.pathname.startsWith(item.path)&&item.path!=="/instructor-dashboard");
              return(
      ...
      <main className="ml-60 flex-1 flex flex-col min-h-screen">
        <header className="bg-white px-8 py-3 border-b border-slate-200 flex justify-between items-center sticky top-0 z-10 shadow-sm">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">Manage your courses, assignments, and students.</p>
          </div>
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-3 pl-6 border-l border-slate-200 cursor-pointer hover:bg-slate-50 p-1 pr-2 rounded-full transition-colors">
              <img src="https://ui-avatars.com/api/?name=Instructor&background=1e3a8a&color=fff" alt="Instructor" className="w-8 h-8 rounded-full border border-white shadow-sm"/>
              <div className="hidden md:block">
                <p className="text-xs font-bold text-slate-800 leading-tight">{sessionStorage.getItem("username")||"Instructor"}</p>
