...
function InstructorAssignments(){
  ...
  const fetchSubmissions=async()=>{
    try{
      const res=await fetch("http://127.0.0.1:8000/api/courses/submissions/",{headers:getHeaders()});
      if(res.ok){
  ...
  const handleGradeSubmit=async(e)=>{
    ...
    try{
      const res=await fetch(`http://127.0.0.1:8000/api/courses/submissions/${selectedSubmission.id}/grade/`,{
        method:"POST",
        headers:getHeaders(),
  ...
  const handlePublishAssignment=async(e)=>{
    ...
    try{
      const res=await fetch(`http://127.0.0.1:8000/api/courses/${newCourseId}/assessments/`,{
        method:"POST",
        headers:getHeaders(),
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button 
          ...
          className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl text-sm font-semibold transition-all shadow-md hover:shadow-lg flex items-center gap-2"
        >
          <Plus size={18} />
          Create Assignment
        </button>
                ...
                <div className="flex flex-col md:flex-row justify-between gap-6">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold uppercase tracking-wider ${!submission.is_graded?'bg-blue-100 text-blue-700':'bg-emerald-100 text-emerald-700'}`}>
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleGradeSubmit} className="p-6 space-y-4">
              ...
              <div className="pt-4 border-t border-slate-100 flex justify-end gap-3">
                ...
                <button type="submit" className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold transition-colors shadow-md">Submit Grade</button>
              </div>
            </form>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handlePublishAssignment} className="p-6 space-y-4">
              ...
              <div className="pt-4 border-t border-slate-100 flex justify-end gap-3">
                ...
                <button type="submit" className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold transition-colors shadow-md">Publish Assignment</button>
              </div>
            </form>
          </div>
        </div>
