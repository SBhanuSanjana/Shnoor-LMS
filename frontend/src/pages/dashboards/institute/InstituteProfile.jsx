...
function InstituteProfile() {
  ...
  const [profile, setProfile] = useState({ full_name: '', email: '', phone_number: '', role: '', department: 'Executive Board' });
  ...
  const fetchProfile = async () => {
    try {
      const res = await api.get('/api/org-admin/profile');
      setProfile({
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-1">
            <div className="sticky top-6 flex flex-col gap-6 max-h-[calc(100vh-2rem)] overflow-y-auto hide-scrollbar pb-2">
              <div>
                <h2 className="text-2xl font-bold text-slate-800">Administrator Profile</h2>
                <p className="text-slate-500 text-sm mt-1">Manage your personal admin credentials and account security</p>
              </div>
              ...
              <div className="px-6 pb-6 flex flex-col items-center relative">
                ...
                <div className="mt-14 text-center mb-6">
                  <h3 className="text-xl font-bold text-slate-800 leading-tight">{profile.full_name || 'Admin'}</h3>
          ...
          <div className="md:col-span-2 space-y-6">
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50 p-6 rounded-xl border border-slate-100">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 mb-2 uppercase tracking-wider">Admin Email</label>
