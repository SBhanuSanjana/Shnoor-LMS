...
const StudentSubscription = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const plansRes = await api.get('/api/plans?plan_type=learner');
            setAvailablePlans(plansRes.data);
            
            const enrollmentsRes = await api.get('/api/courses/enrollments');
            setEnrollmentsCount(enrollmentsRes.data.length || 0);
            
            const storageRes = await api.get('/api/student/storage');
            setStorageBytes(storageRes.data.totalBytes || 0);
        } catch (err) {
            console.error(err);
        } finally {
            setLoading(false);
    ...
    const handlePaymentSuccess = async () => {
        try {
            ...
            alert("Payment activation failed. Please contact support.");
        }
    };
    ...
    return (
        <div className="max-w-5xl mx-auto p-6 space-y-6">
            <div className="mb-8">
                ...
                <p className="text-sm text-slate-500 font-medium mt-1">Manage your billing, plan details, and usage limits</p>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-6">
                ...
                <div className="md:col-span-2 space-y-6">
                    ...
                    {activeSubscription ? (
                        ...
                        <div className="bg-white rounded-2xl border border-slate-200 p-6 flex items-center justify-between shadow-sm">
                            <div>
                                ...
                                <p className="text-sm text-slate-500 font-medium mt-1">Upgrade to unlock premium features and courses.</p>
                            </div>
                            <button 
                                onClick={() => setShowUpgradeModal(true)}
                    ...
                    <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
                        <h3 className="text-lg font-bold text-blue-950 mb-6">Plan Usage</h3>
                        
                        <div className="space-y-6">
                            <div>
                                <div className="flex justify-between items-end mb-2">
                                    <div>
                                        <p className="text-sm font-bold text-blue-950">Course Enrollments</p>
                                        <p className="text-xs text-slate-500">Active enrollments limit</p>
                                    </div>
                                    <p className="text-sm font-bold text-blue-950">{enrollmentsCount} / Unlimited</p>
                                </div>
                                <div className="w-full h-2.5 bg-slate-100 rounded-full overflow-hidden">
                                    <div className="h-full bg-blue-950 rounded-full transition-all duration-500" style={{ width: `${Math.min(100, (enrollmentsCount / 50) * 100)}%` }}></div>
                                </div>
                            </div>
                            ...
                            <div>
                                <div className="flex justify-between items-end mb-2">
                                    <div>
                                        <p className="text-sm font-bold text-blue-950">Storage Usage</p>
                                        <p className="text-xs text-slate-500">For assignments and project files</p>
                                    </div>
                                    <p className="text-sm font-bold text-blue-950">{(storageBytes / (1024 * 1024)).toFixed(2)}MB / 5GB</p>
                                </div>
                                <div className="w-full h-2.5 bg-slate-100 rounded-full overflow-hidden">
                                    <div className="h-full bg-blue-950 rounded-full" style={{ width: `${Math.min(100, (storageBytes / (5 * 1024 * 1024 * 1024)) * 100)}%` }}></div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <h3 className="text-lg font-bold text-blue-950">Billing History</h3>
                        </div>
                ...
                <div className="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl w-full max-w-2xl shadow-xl overflow-hidden animate-fade-in-up flex flex-col max-h-[90vh]">
                        <div className="px-6 py-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center shrink-0">
                            <h3 className="text-lg font-bold text-blue-950">Choose a Plan</h3>
